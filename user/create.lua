...
local function login_heartbeat()
    local os_time = os.time()
    local payload_tmp = pack.pack(">H<i", 0, os_time)
    payload_tmp = payload_tmp .. _G.VERSION
    log.info("payload_tmp", payload_tmp:toHex())
...
local function mqttTask(cid, pios, reg, convert, passon, upprot, dwprot, keepAlive, timeout, addr, port, usr, pwd, cleansession, sub, pub, qos, retain, uid, clientID, addImei, ssl, will, cert)
    ...
    while true do
        ...
        -- 初始化订阅主题
		log.info("----usr, pwd:", usr, pwd)
		log.info("----keepAlive,timeout",keepAlive,timeout)
		_G.mqtt_connected = 1
		mqttc:publish("G/"..misc.getImei().."/Evt_LogIn", login_heartbeat())
        if mqttc:subscribe(sub, qos) then
            if loginMsg(reg) then mqttc:publish(pub[1], loginMsg(reg), tonumber(pub[2]) or qos, retain) end
            local gw_mi = nil
            while true do
                datalink = true
                local r, packet, param = mqttc:receive((keepAlive or 180) * 1000, "NET_SENT_RDY_" .. (passon and cid or uid))
                if gw_mi then log.info("gw_mi:toHex():",gw_mi:toHex()) end
                if r then
                    log.info("订阅的消息:", packet and packet.topic)
                    
                    isACK = string.find(packet.topic, "ACK", 1)
                    isGWCmd = string.find(packet.topic, "M/", 1)
                    topic_flag = string.find(packet.topic, "/", 5)
                    topic_tmp = string.sub(packet.topic,topic_flag+1)
                    isRestart = string.find(packet.topic, "restart", 1)
                    isRspLogIn = string.find(packet.topic, "Rsp_LogIn", 1)
                    
                    if topic_tmp == "ACK" then topic_tmp = nil end
                    if gw_mi then	log.info(gw_mi:toHex()) end                    
                    log.info("packet.payload Hex:",packet.payload:toHex())
                    log.info("convert:", convert,upprotFnc,dwprotFnc)
                    
                    if isRestart then	
                    	log.info("system restart")
                    	
                    	os_time = os.time()
                        log.info("os_time:",os_time)
                        payload_tmp = pack.pack(">H<ib", 0, os_time,0)
                        if gw_mi then
	                        payload_tmp = pack.pack(">bb<ib", string.byte(gw_mi,1), string.byte(gw_mi,2),os_time,0)
                        	gw_mi = nil
                        end
                        
                    	mqttc:publish("G/"..misc.getImei().."/restart", payload_tmp)
                    	log.info(payload_tmp:toHex())
                    	sys.wait(1000)
                    	sys.restart("RECV RESTART CMD...")
                    end
		    
                    messageId = packet.topic:match(".+/rrpc/request/(%d+)")
                    -- 这里执行用户自定义的指令
                    if isACK then
                        log.warn('ACK package...')
                    elseif isRspLogIn then
                    	log.warn('Rsp_LogIn package...')
	                elseif isGWCmd then
	                	payload = string.sub(packet.payload,7)
	                	gw_mi = string.sub(packet.payload, 1, 2)
	                    log.info("gw_mi:toHex():",gw_mi:toHex())
                        sys.publish("UART_SENT_RDY_" .. uid, uid, payload)
                    elseif packet.payload:sub(1, 5) == "rrpc," then
                ...
                elseif packet == 'timeout' then
                    ...
                    log.info("topic", "G/"..misc.getImei().."/Evt_HeartBeat")
                    mqttc:publish("G/"..misc.getImei().."/Evt_HeartBeat", login_heartbeat())                    
                elseif packet == ("NET_SENT_RDY_" .. (passon and cid or uid)) then
                	_G.uart_read_count[uid] = 0
                    if convert == 1 then -- 转换为Hex String 报文
                    ...
                    elseif convert == 0 and upprotFnc then -- 转换为用户自定义报文
                        ...
                        if topic_tmp then
                        	pub_flag = string.find(pub_topic, "/", 5)
                        	pub_topic = string.format("%s%s",string.sub(pub_topic,1,pub_flag),topic_tmp)
                        elseif _G.uart_operation then
                        	pub_flag = string.find(pub_topic, "/", 5)
                        	pub_topic = string.format("%s%s",string.sub(pub_topic,1,pub_flag),_G.uart_operation)
                        end
                        log.info("---1--发布的主题:", pub_topic)
                        topic_tmp = nil
                        os_time = os.time()
                        log.info("os_time:",os_time)
                        payload_tmp = pack.pack(">H<i", 0, os_time)
                        if gw_mi then
	                        payload_tmp = pack.pack(">bb<i", string.byte(gw_mi,1), string.byte(gw_mi,2),os_time)
	                        log.info("--1--gw_mi:toHex()",gw_mi:toHex())
                        	gw_mi = nil
                        end
                        paylog_data = payload_tmp..param
                        log.info(paylog_data:toHex())
                        if not mqttc:publish(pub_topic, paylog_data, tonumber(pub[index + 1]) or qos, retain) then if passon then sys.publish("UART_SENT_RDY_" .. uid, uid, "SEND_ERROR\r\n") end break end
                    else
                        ...
                        if gw_mi then
	                        payload_tmp = pack.pack(">bb<i", string.byte(gw_mi,1), string.byte(gw_mi,2),os_time)
	                        log.info("--2--gw_mi:toHex()",gw_mi:toHex())
                        	gw_mi = nil
                        end
                        paylog_data = payload_tmp..param
                        log.info(paylog_data:toHex())
                        if not mqttc:publish(pub_topic, paylog_data, tonumber(pub[2]) or qos, retain) then if passon then sys.publish("UART_SENT_RDY_" .. uid, uid, "SEND_ERROR\r\n") end break end
                    end
                    messageId = false
                    if passon then sys.publish("UART_SENT_RDY_" .. uid, uid, "SEND_OK\r\n") end
