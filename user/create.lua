...
---------------------------------------------------------- lnxall 通用应答 ----------------------------------------------------------
local function jjGeneralAck(recv_payload,err_code,err_msg)
    local r_obj = json.decode(recv_payload)
    local ack = nil
    if r_obj then
        ack = json.encode({mi = r_obj.mi or 999,timestamp = os.time(), error_code = err_code or 0})
    end
    return ack
end

---------------------------------------------------------- lnxall 控制应答 ----------------------------------------------------------
local function jjControlAck(recv_payload,_status)
    local r_obj = json.decode(recv_payload)
    local ack = nil
    if r_obj then
        ack = json.encode({mi = r_obj.mi or 999,timestamp = os.time(), status = _status or 0})
    end
    return ack
end
            ...
            while true do
                ...
                elseif packet == ("NET_SENT_RDY_" .. "JJIOT") then
                    for i = 0,#recvBuff do
                        ...
                        if packet then
                            log.info("publish message:", packet[1],packet[2])
                            if not mqttc:publish(packet[1],packet[2], 2) then
                                log.warn("miss message:",packet[1],packet[2])
                            end
                            packet = nil
