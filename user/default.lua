...
-- 判断模块类型
_G.mqtt_connected = 0
_G.uart_read_count = {0, 0}
local mqtt_connected_fail = 0
...
local function autoSampl(uid, t)
	local collect_wait = tonumber(t[1])
	if collect_wait < 200 then collect_wait = 200 end
    while true do
        sys.waitUntil("AUTO_SAMPL_" .. uid)
        --log.info("collect_wait",collect_wait,tonumber(t[1]))
        for i = 2, #t do
            local str = t[i]:match("function(.+)end")
            
            local opt_flag = string.find(t[i], ":", 1)
	        local send_data = nil
	        if opt_flag then
	        	_G.uart_operation = string.sub(t[i], 1, opt_flag-1)
	        	send_data = string.sub(t[i], opt_flag+1)
	        	log.info("_G.uart_operation:",_G.uart_operation);
	        end
	    
            if not str then
                if t[i] ~= "" then 
                	if opt_flag then 
                		write(uid, (send_data:fromHex()))
                		log.info("send_data:toHex():", send_data:fromHex():toHex())
                	else
                		write(uid, (t[i]:fromHex()))
                		log.info("t[i]:toHex():", t[i]:fromHex():toHex())
                	end
                end
            else
...
local function read(uid)
    ...
    _G.uart_read_count[uid] = _G.uart_read_count[uid] + 1
...
sys.timerLoopStart(function()
	 log.info("mqtt_connected mqtt_connected_fail:",_G.mqtt_connected,mqtt_connected_fail)
     log.info("_G.uart_read_count[1 2]:",_G.uart_read_count[1],_G.uart_read_count[2])
	 log.info("is4gLod,is1802S",is4gLod,is1802S)
     log.info("打印占用的内存:", _G.collectgarbage("count"))-- 打印占用的RAM
     log.info("打印可用的空间", rtos.get_fs_free_size())-- 打印剩余FALSH，单位Byte
     socket.printStatus()
     if _G.mqtt_connected ~= 1 then mqtt_connected_fail = mqtt_connected_fail + 1 end
     if mqtt_connected_fail >= 5 then
     	log.info("The MQTTServer connection is broken...")
     	sys.wait(1000)
     	sys.restart("The MQTTServer connection is broken...")
     end
     
     if _G.uart_read_count[1] >= 5 or _G.uart_read_count[2] >= 5 then 
     	sys.wait(1000)
     	sys.restart("UART DATA PROCESS ERROR...")
     end
 end, 60000)
...
---------------------------------------------------------- LED 控制 ---------------------------------------------------------
--RunLed控制
function taskRunLed(runLedPinSetFunc)
	local run_led_switch = 1
    while true do
    	--log.info("run_led_switch",run_led_switch)
        if run_led_switch == 1 then
            runLedPinSetFunc(1)
            run_led_switch = 0
        else
            runLedPinSetFunc(0)    
...
sys.taskInit(taskRunLed, pins.setup(pio.P0_27, 0))
