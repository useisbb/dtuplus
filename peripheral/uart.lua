...
function uart.setup(id, baud, databits, parity, stopbits,msgmode,txDoneReport)
    if not id or  not baud or not databits or not parity or not stopbits then
        log.error("peripheral-uart","串口配置参数错误",id, baud, databits, parity, stopbits)
        return
    end
    log.debug(string.format("串口[%d]参数配置",id))
    log.info("peripheral-uart","baud=" .. id)
    log.info("peripheral-uart","baud=" .. databits)
    log.info("peripheral-uart","baud=" .. parity)
    log.info("peripheral-uart","baud=" .. stopbits)
